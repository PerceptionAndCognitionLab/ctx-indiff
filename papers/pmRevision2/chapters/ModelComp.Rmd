...
Once the predictions are derived, model comparison is as simple as comparing these predictive densities at observed values.  Suppose the individuals have observed effects of $d_1 = 150$ ms and $d_2 = 100$ ms.  These values are shown by the point in panels $A_2$ through $D_2$.  The density for $(150, 100)$ under the unstructured model $\calM_u$ is `r paste(round(pN, 5))`. The density under the positive-effects model for the same data, $\calM_+$, is `r paste(round(pTN, 5))`. The remaining densities are `r paste(round(p1, 5), "and", round(p0, 5))` for models  $\calM_1$ and $\calM_0$, respectively. We use the ratios of these densities to compare the models. The ratio of the densities for models $\calM_+$ and $\calM_u$, for example, is `r round(pTN/pN)`-to-1 in favor of $\calM_+$. This ratio is the Bayes factor and is denoted $B_{+u}$.  The comparison of all models may be done in the same manner. For example, the Bayes factor between the positive-effects model and the null-effect model is `r round(pTN/p0)`-to-1 in favor of $\calM_+$. 
